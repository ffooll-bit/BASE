...
- **Graduation wizard thesis grade fill targeted the wrong semester (ENH-010):** when a student had several thesis/skripsi rows across semesters (e.g. retakes), card 3 initially tagged *every* missing-grade thesis row "(akan diisi)" and `finish()` could push the Excel grade into an *older* row (e.g. filling `20251` when `20252` was the real last thesis). Now the grade targets a single row — the thesis/skripsi in the **last semester** (`id_smt` largest) — and only while that last-semester row is **still ungraded**; if the last-semester thesis already has a grade, no value is filled and no older row is backfilled. Card 3 tags "(akan diisi)" only on that one row; every other ungraded row renders "Belum ada nilai" — #113

- **Graduation wizard `finish()` fatal TypeError on submit:** `Graduation::finish()` was declared `: string` but returns a `RedirectResponse` (`redirect()->to('graduation/guidance')`), throwing `TypeError: Return value must be of type string` the moment the wizard is actually submitted to Neo Feeder. Changed the return type to `CodeIgniter\HTTP\RedirectResponse`. Also, the full submission iterates the whole batch with a synchronous call chain per student (InsertMahasiswaLulusDO + academic edits + thesis grade), which could exceed PHP's default 60 s limit on large batches → added `set_time_limit(0)` at the top of `finish()` — #113

- **Graduation wizard academic edits silently rejected by Neo Feeder (#117):** `Graduation::finish()` pushed per-semester academic corrections (status / IPK / IPS) as a *partial* `UpdatePerkuliahanMahasiswa` record (only `id_status_mahasiswa`, `ips`, `ipk`). Neo Feeder rejects partial records — it enforces `id_status_mahasiswa`, `biaya_kuliah_smt` and `id_pembiayaan` even though the GetDictionary schema lists only `id_status_mahasiswa` as NOT NULL — so every academic edit failed and the last-semester IPK/status was never actually updated. The failure was also invisible because the response was discarded (no result row). `finish()` now sends the **full** activity row (minus the composite key) plus `id_pembiayaan` resolved via `GetListRiwayatPendidikanMahasiswa`, with only the edited fields overridden — the same record shape as the Verifikasi IPK menu — and records a per-semester success/failure result so the guidance page shows what actually happened — #117
