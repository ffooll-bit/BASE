...
### ENH-013 — Fitur alur kelulusan mahasiswa PISN (upload Excel + verifikasi manual berurutan)
- **Status:** `verified`
- **Issue:** `—`
- **Recorded:** 2026-08-27
- **Implemented:** `—`
- **Problem:** Admin memproses kelulusan mahasiswa untuk PISN (Penomoran Ijazah dan Sertifikat Profesi Nasional) secara manual di Neo Feeder dengan alur berbilang langkah dan gerbang verifikasi ketat. Tidak ada antarmuka terstruktur di BASE yang memandu alur ini, sehingga rawan salah — terutama karena koreksi data pasca-lulus (KTP/Nama/Jenis Kelamin) memerlukan surat resmi ke LLDIKTI. Bukti (KTP, transkrip) masih berupa gambar, sehingga verifikasi tidak bisa otomatis. Alur manual saat ini (sudut pandang admin Neo Feeder): (1) verifikasi identitas — KTP, Nama, Jenis Kelamin via *Mahasiswa → Daftar Mahasiswa → cari NIM/NPM*; ketiga field wajib benar sebelum lulus karena koreksi pasca-lulus butuh menyurat ke LLDIKTI; (2) verifikasi akademik — *Perkuliahan → Aktifitas Kuliah Mahasiswa → cari di periode semester kelulusan*; pastikan status aktif di semester itu, IPK & SKS cocok transkrip; koreksi langsung saat itu juga, dan mengubah data mewajibkan *Biaya Kuliah Semester* terisi (tidak boleh kosong); (3) cek eligibility di aplikasi PISN — API PISN dilaporkan sudah tersedia tapi masih menunggu kepastian dari admin LLDIKTI, untuk sekarang admin cek manual; jika tidak eligible, data harus disesuaikan; (4) input kelulusan — Neo Feeder → *Perkuliahan → Daftar Mahasiswa Lulus/Dropout → tambah mahasiswa*, isi NPM, Nama, Jenis Keluar, Tanggal Keluar/Tanggal Lulus, Periode Keluar, IPK, dan No Ijazah/No Sertifikat Profesi diisi "-"; (5) pasca-lulus — tunggu beberapa hari sampai data sinkron ke PISN, lalu buat nomor Ijazah di aplikasi web PISN.
- **Possible Fix:** Bangun fitur di BASE yang meniru alur Neo Feeder dengan dua penambahan dari masukan user: (a) **upload data via Excel** — admin upload file Excel berisi data mahasiswa calon lulus; (b) **verifikasi manual berurutan** — karena bukti (KTP, transkrip, dll masih berbentuk gambar) tidak bisa diverifikasi otomatis, admin tetap cek satu per satu, tapi antarmuka dibuat sekuensial dengan tombol **Next** untuk lanjut ke data berikutnya. Rincian langkah: (1) upload Excel → siapkan antrian data; (2) wizard verifikasi identitas per-item (KTP/Nama/Jenis Kelamin) via Daftar Mahasiswa, klik Next per item; (3) wizard verifikasi akademik per-item (status aktif, IPK, SKS) via Aktifitas Kuliah Mahasiswa, dengan validasi Biaya Kuliah Semester wajib terisi saat edit; (4) cek eligibility PISN secara manual — **API PISN ditunda (tahap manual dulu) sambil menunggu konfirmasi LLDIKTI, tapi scaffolding integrasi API disiapkan agar tinggal disambung bila tersedia**; (5) input kelulusan via Daftar Mahasiswa Lulus/Dropout (NPM, Nama, Jenis Keluar, Tanggal Keluar/Lulus, Periode Keluar, IPK, No Ijazah/No Sertifikat Profesi = "-"); (6) panduan pasca-lulus (tunggu sinkron PISN, buat nomor Ijazah di web PISN). Verifikasi tetap manual satu per satu; sistem hanya mempermudah navigasi dan input.
- **Actual Fix:** Feasible via NeoFeeder WS API (GetListMahasiswa, GetAktivitasKuliahMahasiswa, InsertMahasiswaLulusDO) + PISN. PISN API cek eligibilitas ada (daftar via bit.ly/FormAPIPISN, dokumen diemail) → integrasi langsung ditunda, siapkan scaffolding (service interface + adapter seam) agar API tinggal disambung. Upload Excel selaras dengan metode "Upload file excel" milik PISN. Implementasi: tambah phpoffice/phpspreadsheet untuk parse Excel; bangun wizard verifikasi berurutan (upload Excel → cek identitas → cek akademik dengan validasi Biaya Kuliah Semester → cek eligibility PISN manual/API → input kelulusan → panduan pasca-lulus). Tidak duplikat (BASE belum punya fitur kelulusan).
- **Actual Implemented:** `—`
- **Changes:** `—`

### ENH-014 — Halaman menu individu Neo Feeder yang terpakai di alur kelulusan
- **Status:** `verified`
- **Issue:** `—`
- **Recorded:** 2026-08-27
- **Implemented:** `—`
- **Problem:** Alur kelulusan (ENH-013) membutuhkan akses ke beberapa menu Neo Feeder — *Daftar Mahasiswa*, *Aktifitas Kuliah Mahasiswa*, *Daftar Mahasiswa Lulus/Dropout*. BASE belum memiliki halaman menu mandiri yang meniru navigasi Neo Feeder, sehingga admin harus berpindah ke Neo Feeder untuk tiap langkah verifikasi/input.
- **Possible Fix:** Tambah halaman menu mandiri di BASE yang memetakan menu Neo Feeder yang terpakai di alur ENH-013: *Daftar Mahasiswa*, *Aktifitas Kuliah Mahasiswa*, *Daftar Mahasiswa Lulus/Dropout* (dan menu lain yang muncul saat implementasi ENH-013). Setiap halaman memanggil NeoFeeder API melalui layer service yang sudah ada (`Libraries/NeoFeeder`). Menjadi permukaan navigasi bagi wizard ENH-013.
- **Actual Fix:** Feasible — NeoFeeder WS API ekspos GetListMahasiswa, GetAktivitasKuliahMahasiswa, GetListMahasiswaLulusDO. Tambah halaman read-only di BASE (controller + view + route) memanggil layer Libraries/NeoFeeder yang diperluas dengan 3 method Get tersebut. Menjadi permukaan navigasi wizard ENH-013.
- **Actual Implemented:** `—`
- **Changes:** `—`

### ENH-015 — Operasi CRUD untuk entitas Neo Feeder (opsional)
- **Status:** `verified`
- **Issue:** `—`
- **Recorded:** 2026-08-27
- **Implemented:** `—`
- **Problem:** Halaman menu (ENH-014) awalnya hanya read-only. Beberapa langkah alur kelulusan (ENH-013) memerlukan pengubahan data (mis. koreksi Aktifitas Kuliah Mahasiswa, input kelulusan), sehingga fitur edit/tambah/hapus berguna bila API NeoFeeder mendukung.
- **Possible Fix:** Di luar menu read-only (ENH-014), tambah operasi create/edit/delete untuk entitas NeoFeeder di mana API mendukungnya (periksa dokumentasi NeoFeeder API per endpoint). Ditandai **opsional** oleh user — bisa dikerjakan sebagian atau ditunda. Pastikan tersedia di ENH-014 sebelum menambah CRUD.
- **Actual Fix:** Feasible — NeoFeeder WS API dukung Insert/Update/Delete (InsertBiodataMahasiswa, InsertPerkuliahanMahasiswa, dst). Tambah CRUD di mana API mengizinkan. Opsional — implementasi per ketersediaan endpoint, dibangun di atas halaman ENH-014.
- **Actual Implemented:** `—`
- **Changes:** `—`
