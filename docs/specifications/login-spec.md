...
**Priority**: High
**Description**: The system shall authenticate users by sending a POST request to the Neo Feeder Web Service API. The API endpoint base URL is `https://neofeeder.stiem-bongaya.ac.id/`. All requests shall use `Content-Type: application/json`.
...
- Send a POST request to `https://neofeeder.stiem-bongaya.ac.id/` with `Content-Type: application/json` and body `{"act":"GetToken","username":"...","password":"..."}`
...
**When** they configure the Neo Feeder API endpoint URL (default: `https://neofeeder.stiem-bongaya.ac.id/`) and connection parameters in `.env`
